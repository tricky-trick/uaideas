<html>
<head>
    <title>Про проект "SeeUA"</title>
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta name="keywords" content="Україна,замки,фортеці,подорожі,відпочинок,туризм,подорожувати,провести час,ресторани,кафе,актиний відпочинок">
    <link rel="icon" type="image/png" href="img/favicon-32x32.png" sizes="32x32">
    <link rel="stylesheet" type="text/css" href="css/main.css" />
    <link rel="stylesheet" href="css/normalize.css">
    <link rel="stylesheet" href="css/default.css">
    <link rel="stylesheet" href="css/ideal-image-slider.css">
</head>
<body style="padding-left: 20px; padding-right: 20px; font-family:  tahoma, arial, verdana, sans-serif, 'Lucida Sans';">
    <div id="top-side-panel" style="padding: 10px">
        <a href="index.php"><img src="img/logo_see.png" style="margin-left: 10%; width: auto; height: 100%; border-radius: 5px"></a>
        <h2 style="display:  inline-block; color: #FFFFFF; margin-left: 10%; margin-top: 0%">Про проект</h2>
    </div>
    <br>
    <br>
    <br>
    <br>
    <p style="text-align: justify; font-size: 16px; font-family:  tahoma, arial, verdana, sans-serif, 'Lucida Sans'; margin-left: 10%; margin-right: 10%">"SeeUA" - перший український веб ресурс, котрий дозволяє з легкістю знайти чи додати цікаві місця для часопроведення в Україні.
        Жодної реклами, жодної комерції. Лише корисна інформація.
        Замки, музеї, місця для фотосесій або ж просто чудові краєвиди, місця для актиного відпочинку на велосипедах чи цілою сім'єю, затишні кафе чи ресторани
        - тепер все на одному ресурсі. Ви зможете побачити коментрарі та оцінки людей. Зможете самі додати свої враження.
        Головною метою даного ресурсу є показати не лише відомі цікаві місця в різних населених пунктах, але й ті місця, про які знають лише жителі цих міст.
        Для користування даним ресурсом можна скористатися демо-входом або ж пройти просту реєстрацію чи зайти за допомогою існуючого аккаунта соціальної мережі. В разі демо-входу не будуть доступні можливості редагування чи додавання інформації чи коментарів.
        Користуйтеся ресурсом, додавайте нові місця і діліться враженнями з іншими.
    </p>
    <br>
    <br>
    <div id="slider" style="width: 80%; margin-left: 10%;">
        <img src="img/img1.jpg" />
        <img src="img/img2.jpg" />
        <img src="img/img3.jpg" />
        <img src="img/img4.jpg" />
    </div>

    <script src="js/ideal-image-slider.js"></script>
    <script>
        var slider = new IdealImageSlider.Slider('#slider');
        slider.start();
    </script>
    <br>
    <div style="width:10%; text-align: center; margin-left: 45%; font-size: 12px">
        SeeUA © 2015
    </div>
</body>
</html>